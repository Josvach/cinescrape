...
/**
 * The programme page embeds Nuxt's payload as a flat array where object values
 * are indices into that same array. Resolving one level of indirection is all
 * we need for the event records.
 */
export function parseProgramme(html: string): ScheduledEvent[] {
  ...
  const out = new Map<string, ScheduledEvent>();
  for (const entry of flat) {
    if (!entry || typeof entry !== "object" || Array.isArray(entry)) continue;
    const rec = entry as Record<string, unknown>;
    if (!("EventId" in rec) || !("Start" in rec)) continue;

    const eventId = deref(rec.EventId);
    const start = deref(rec.Start);
    const titleId = deref(rec.TitleId);
    const objectId = deref(rec.ObjectId);
    if (typeof eventId !== "string" || typeof start !== "string") continue;
...
export async function fetchProgramme(slug: string): Promise<ScheduledEvent[]> {
